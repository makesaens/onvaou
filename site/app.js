...
(function () {
  ...
  if (tous.length) carte.fitBounds(L.latLngBounds(tous), { paddingTopLeft: [40, 40], paddingBottomRight: [40, 190] }); else carte.setView([46.5, -70], 5);
  ...
  /* ---------- l'intro en boucle dans le hero (lecteur YouTube officiel, passage 1:58 → 3:12) ---------- */
  const hv = $('#hero-video');
  if (hv) {
    const id = hv.dataset.video, deb = +hv.dataset.debut, fin = +hv.dataset.fin;
    const reduit = matchMedia('(prefers-reduced-motion: reduce)').matches;
    if (reduit) { hv.innerHTML = `<img src="https://i.ytimg.com/vi/${id}/maxresdefault.jpg" alt="">`; }
    else {
      let player;
      const monter = () => { player = new YT.Player('yt-intro', { videoId: id, playerVars: { autoplay: 1, mute: 1, controls: 0, start: deb, end: fin, modestbranding: 1, rel: 0, playsinline: 1, disablekb: 1, iv_load_policy: 3, fs: 0, cc_load_policy: 0 }, events: { onReady: (e) => { e.target.mute(); e.target.playVideo(); }, onStateChange: (e) => { if (e.data === YT.PlayerState.ENDED) { e.target.seekTo(deb, true); e.target.playVideo(); } } } }); };
      if (window.YT && window.YT.Player) monter(); else window.onYouTubeIframeAPIReady = monter;
    }
  }
